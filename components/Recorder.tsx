...
import { FaMicrophone, FaStop, FaRedo, FaPlay, FaPause, FaVolumeUp, FaCheck } from 'react-icons/fa';
                    ...
                    <div className="w-full flex flex-col items-center mt-4 gap-2">
                        ...
                        <div className="flex gap-4 justify-center items-center mb-2">
                            <Button
                                ...
                                ripple
                                className="rounded-full w-14 h-14 flex items-center justify-center text-2xl shadow-lg bg-gradient-to-br from-emerald-200 via-emerald-100 to-white/80"
                            >
                                {isPlaying ? <FaPause /> : <FaPlay />}
                            </Button>
                            {onSubmit && (
                                <Button color="blue" onClick={() => onSubmit && audioUrl && fetch(audioUrl).then(r => r.blob()).then(onSubmit)} ripple className="rounded-full w-12 h-12 flex items-center justify-center text-xl shadow-lg bg-gradient-to-br from-blue-200 via-blue-100 to-white/80" title="提交录音"> <FaCheck /> </Button>
                            )}
                            <Button color="amber" onClick={reset} ripple className="rounded-full w-12 h-12 flex items-center justify-center text-xl shadow-lg bg-gradient-to-br from-amber-200 via-amber-100 to-white/80" title="重置"> <FaRedo /> </Button>
                        </div>
                    ...
                    <div className="w-full mt-4">
                        ...
                        <div className="flex flex-col gap-2">
                            {history.slice(1).map((h, idx) => (
                                <div key={idx} className="flex items-center gap-3 bg-white/60 rounded-xl px-3 py-2 shadow-glass">
                                    <div className="flex gap-2">
                                        <Button color="gray" ripple onClick={() => {
                                            setAudioUrl(h.url);
                                            setIsPlaying(false);
                                            setAudioProgress(0);
                                        }} className="rounded-full w-8 h-8 flex items-center justify-center text-base" title="回放"> <FaPlay /> </Button>
                                        {onSubmit && (
                                            <Button color="blue" ripple onClick={() => onSubmit(h.blob)} className="rounded-full w-8 h-8 flex items-center justify-center text-base" title="提交"> <FaCheck /> </Button>
                                        )}
                                    </div>
