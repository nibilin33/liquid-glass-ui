...
export function ClozeInput({
  ...
  const checkAndAutoSubmit = (nextInputs: any) => {
    ...
    if (!letterMode) {
      ...
      filled = nextInputs.every((arr: string[]) =>
        arr.every((v: string) => v && v.length > 0)
      );
    }
    if (filled && !submitted) {
  ...
  const handleChange = (blankIdx: number, letterIdx: number, val: string) => {
    if (!letterMode) {
      ...
      // prefill 不可修改
      if (prefill && prefill[blankIdx] && prefill[blankIdx][letterIdx] != null)
        return;
  ...
  const handleSubmit = (inputOverride?: any) => {
    ...
    if (!letterMode) {
      ...
      isCorrect = useInputs.every(
        (arr: string[], i: number) =>
          arr.join("").trim().toLowerCase() === answers[i].trim().toLowerCase()
      );
    }
    setSubmitted(true);
    ...
    if (!letterMode) {
      ...
      onSubmit?.(
        useInputs.map((arr: string[]) => arr.join("")),
        isCorrect
      );
    }
  };
            ...
            return (
              <motion.input
                ...
                style={{
                  ...
                  width: `${Math.max(
                    2.5,
                    (inputs[idx] as string)?.length || 2.5
                  )}em`,
            ...
            return (
              <span key={i} className="inline-flex gap-1 align-middle">
                {(answers[idx] as string).split("").map((_, letterIdx) => {
                  const isPrefilled =
                    prefill && prefill[idx] && prefill[idx][letterIdx] != null;
                  return (
                    <motion.input
                      key={letterIdx}
                      type="text"
